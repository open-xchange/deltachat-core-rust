...
impl LoginParam {
    ...
    pub fn from_database(context: &Context, prefix: impl AsRef<str>) -> Self {
        ...
        let auth_scheme = AuthScheme::from_str(sql.get_raw_config(context, key).unwrap_or_default().as_str()).unwrap_or_default();
        let key = format!("{}imap_security", prefix);
        let imap_security = ServerSecurity::from_str_opt(sql.get_raw_config(context, key).unwrap_or_default().as_str());
        let key = format!("{}smtp_security", prefix);
        let smtp_security = ServerSecurity::from_str_opt(sql.get_raw_config(context, key).unwrap_or_default().as_str());
    ...
    pub fn save_to_database(
        ...
        let key = format!("{}mail_server", prefix);
        sql.set_raw_config(context, key, Some(&self.srv_params[Service::Imap as usize].hostname))?;

        let key = format!("{}mail_port", prefix);
        sql.set_raw_config_int(context, key, self.srv_params[Service::Imap as usize].port)?;

        let key = format!("{}mail_user", prefix);
        sql.set_raw_config(context, key, Some(&self.srv_params[Service::Imap as usize].user))?;

        let key = format!("{}mail_pw", prefix);
        sql.set_raw_config(context, key, Some(&self.srv_params[Service::Imap as usize].pw))?;

        let key = format!("{}imap_certificate_checks", prefix);
        sql.set_raw_config_int(context, key, self.srv_params[Service::Imap as usize].certificate_checks as i32)?;

        let key = format!("{}send_server", prefix);
        sql.set_raw_config(context, key, Some(&self.srv_params[Service::Smtp as usize].hostname))?;

        let key = format!("{}send_port", prefix);
        sql.set_raw_config_int(context, key, self.srv_params[Service::Smtp as usize].port)?;

        let key = format!("{}send_user", prefix);
        sql.set_raw_config(context, key, Some(&self.srv_params[Service::Smtp as usize].user))?;

        let key = format!("{}send_pw", prefix);
        sql.set_raw_config(context, key, Some(&self.srv_params[Service::Smtp as usize].pw))?;

        let key = format!("{}smtp_certificate_checks", prefix);
        sql.set_raw_config_int(context, key, self.srv_params[Service::Smtp as usize].certificate_checks as i32)?;
        ...
        if self.srv_params[Service::Imap as usize].security.is_some() {
            ...
            let val = if self.srv_params[Service::Imap as usize]
                .security
                .is_none() {
                    None
                } else {
                    sec = self
                        .srv_params[Service::Imap as usize]
                        .security
                        .unwrap();
                    Some(sec.as_ref())
                };
            println!("Saving imap_sec: {:?}", val);
        ...
        if self.srv_params[Service::Smtp as usize].security.is_some() {
            ...
            let sec: ServerSecurity;
            let val = if self.
                srv_params[Service::Smtp as usize]
                .security
                .is_none() {
                    None
                } else {
