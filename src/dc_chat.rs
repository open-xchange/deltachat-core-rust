...
pub unsafe fn dc_create_chat_by_msg_id(
    ...
    if !(context.is_null() || (*context).magic != 0x11a11807i32 as libc::c_uint) {
        if !(0 == dc_msg_load_from_db(msg, context, msg_id)
            || 0 == dc_chat_load_from_db(context, chat, (*msg).chat_id)
...
pub unsafe fn dc_chat_new() -> *mut dc_chat_t {
    let mut chat = calloc(1, ::std::mem::size_of::<dc_chat_t>()) as *mut dc_chat_t;
    if chat.is_null() {
        exit(14i32);
    }
...
pub unsafe fn dc_chat_load_from_db(
    ...
    if !(chat.is_null() || (*chat).magic != 0xc4a7c4a7u32) {
        ...
        if !(sqlite3_step(stmt) != 100i32) {
            if !(0 == set_from_stmt(context, chat, stmt)) {
                success = 1i32
            }
        }
    }
...
pub unsafe fn dc_create_or_lookup_nchat_by_contact_id(
    ...
    contact = dc_contact_new();
    if !(0 == dc_contact_load_from_db(context, contact, contact_id)) {
        chat_name =
...
unsafe fn prepare_msg_common(
    ...
    match current_block {
        17281240262373992796 => {
            ...
            chat = dc_chat_new();
            if 0 != dc_chat_load_from_db(context, chat, chat_id) {
                if (*msg).state != 18i32 {
                    (*msg).state = 20i32
                }
                (*msg).id =
                ...
                _ => {
                    ...
                    if 0 != e2ee_enabled && dc_param_get_int((*msg).param, 'u' as i32, 0i32) == 0i32
                    {
                        ...
                        if 0 != can_encrypt {
                            ...
                            } else if 0 != last_msg_in_chat_encrypted(context, (*chat).id) {
                                do_guarantee_e2ee = 1i32
                            }
                        }
                    }
 ...
 * Sending messages
 ******************************************************************************/
unsafe fn last_msg_in_chat_encrypted(context: *mut dc_context_t, chat_id: uint32_t) -> libc::c_int {
...
pub unsafe fn dc_chat_update_param(
    context: *mut dc_context_t,
    chat: *mut dc_chat_t,
) -> libc::c_int {
    let mut success: libc::c_int = 0i32;
...
pub unsafe fn dc_get_chat_msgs(
    ...
    //clock_t       start = clock();
    let mut success: libc::c_int = 0i32;
    let mut ret: *mut dc_array_t = dc_array_new(512i32 as size_t);
    let mut stmt: *mut sqlite3_stmt = 0 as *mut sqlite3_stmt;
...
pub unsafe fn dc_delete_chat(mut context: *mut dc_context_t, mut chat_id: uint32_t) {
    ...
    let mut obj: *mut dc_chat_t = dc_chat_new();
    let mut q3: *mut libc::c_char = 0 as *mut libc::c_char;
    if !(context.is_null()
        || (*context).magic != 0x11a11807i32 as libc::c_uint
        || chat_id <= 9i32 as libc::c_uint)
    {
        if !(0 == dc_chat_load_from_db(context, obj, chat_id)) {
        ...
        || chat_id <= 9i32 as libc::c_uint)
    {
        ...
        if !(0i32 == real_group_exists(context, chat_id)
            || 0i32 == dc_real_contact_exists(context, contact_id)
                && contact_id != 1i32 as libc::c_uint
            || 0i32 == dc_chat_load_from_db(context, chat, chat_id))
        {
                ...
                if !(strcasecmp((*contact).addr, self_addr) == 0i32) {
                    ...
                    if 0 != dc_is_contact_in_chat(context, chat_id, contact_id) {
                        ...
                        if (*chat).type_0 == 130i32 {
                            if dc_contact_is_verified(context, contact) != 2i32 {
            ...
            if !(dc_is_contact_in_chat(context, chat_id, 1i32 as uint32_t) == 1i32) {
                ...
                match current_block {
                    ...
                    _ => {
                        ...
                        if !(0 == dc_chat_update_param(context, chat)) {
                            if dc_param_get_int((*chat).param, 'U' as i32, 0i32) == 0i32 {
...
pub unsafe extern "C" fn dc_chat_get_subtitle(
    context: *mut dc_context_t,
    ...
    if (*chat).type_0 == 100i32 && 0 != dc_param_exists((*chat).param, 'K' as i32) {
        ret = dc_stock_str(context, 50i32)
        ...
        let mut cnt: libc::c_int = 0i32;
        if (*chat).id == 1i32 as libc::c_uint {
            ret = dc_stock_str(context, 8i32)
        } else {
            cnt = dc_get_chat_contact_cnt(context, (*chat).id);
            ret = dc_stock_str_repl_int(context, 4i32, cnt)
        }
    }
...
pub unsafe fn dc_chat_get_profile_image(
    context: *mut dc_context_t,
    mut chat: *const dc_chat_t,
) -> *mut libc::c_char {
    ...
    if !(chat.is_null() || (*chat).magic != 0xc4a7c4a7u32) {
        ...
        if !image_rel.is_null() && 0 != *image_rel.offset(0isize) as libc::c_int {
            image_abs = dc_get_abs_path(context, image_rel)
        } else if (*chat).type_0 == 100i32 {
            contacts = dc_get_chat_contacts(context, (*chat).id);
            if (*contacts).count >= 1 {
                contact = dc_get_contact(context, *(*contacts).array.offset(0isize) as uint32_t);
                image_abs = dc_contact_get_profile_image(context, contact)
            }
        }
    }
...
pub unsafe fn dc_chat_get_color(context: *mut dc_context_t, chat: *const dc_chat_t) -> uint32_t {
