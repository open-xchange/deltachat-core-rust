...
unsafe extern "C" fn poke_public_key(
    ...
    if !(addr.is_null() || public_key_file.is_null() || peerstate.is_null() || header.is_null()) {
            ...
            || 0 == dc_pgp_is_valid_key(context, (*header).public_key)
        {
            ...
            if 0 != dc_apeerstate_load_by_addr(peerstate, (*context).sql, addr) {
    ...
    while i < dc_array_get_cnt(contacts) {
        ...
        if !contact.is_null() {
            ...
            let mut peerstate_ok: libc::c_int =
                dc_apeerstate_load_by_addr(peerstate, (*context).sql, addr);
...
pub unsafe extern "C" fn dc_cmdline(
    ...
    if !(context.is_null() || cmdline.is_null() || *cmdline.offset(0isize) as libc::c_int == 0i32) {
        ...
        {
            if !arg1.is_null()
                && strcmp(arg1, b"imex\x00" as *const u8 as *const libc::c_char) == 0i32
            {
                ret =
            ...
            if strcmp(cmd, b"auth\x00" as *const u8 as *const libc::c_char) == 0i32 {
                ...
                if strcmp(arg1, is_pw) == 0i32 {
                    ...
                    ret = b"Bad password.\x00" as *const u8 as *const libc::c_char
                        as *mut libc::c_char
                }
            } else {
                ret = dc_strdup(
        ...
        ) == 0i32
        {
            if !arg1.is_null() {
                ...
                if 0 != dc_msg_is_setupmessage(msg) {
                    ...
                    ret = dc_mprintf(
                        b"ERROR: Msg#%i is no setup message.\x00" as *const u8
                            as *const libc::c_char,
                        msg_id,
                    )
                }
        ...
        ) == 0i32
        {
            if !arg1.is_null() {
                ...
                ret = dc_strdup(
                    b"ERROR: Argument <backup-file> missing.\x00" as *const u8
                        as *const libc::c_char,
                )
            }
            ...
            if !arg1.is_null() {
                ...
                if bits > 15i32 {
                    ret = dc_strdup(
                        b"ERROR: <bits> must be lower than 16.\x00" as *const u8
                            as *const libc::c_char,
                    )
                } else {
            ...
            if !arg1.is_null() {
                ...
                ret = dc_strdup(
                    b"ERROR: Argument <key> missing.\x00" as *const u8 as *const libc::c_char,
                )
            }
            ...
            || strcmp(cmd, b"chats\x00" as *const u8 as *const libc::c_char) == 0i32
        {
            ...
            if !arg1.is_null() && 0 != *arg1.offset(0isize) as libc::c_int {
                ...
                (*context).cmdline_sel_chat_id = atoi(arg1) as uint32_t;
                sel_chat = dc_get_chat(context, (*context).cmdline_sel_chat_id);
                if sel_chat.is_null() {
                    (*context).cmdline_sel_chat_id = 0i32 as uint32_t
                }
            }
            if !sel_chat.is_null() {
        ...
        ) == 0i32
        {
            ...
            if !sel_chat.is_null() {
                if !arg1.is_null() {
                    ...
                    if 0 != dc_add_contact_to_chat(
                        ...
                        ret = dc_strdup(
                            b"Contact added to chat.\x00" as *const u8 as *const libc::c_char,
                        )
                    } else {
                        ret = dc_strdup(
            ...
            if !sel_chat.is_null() {
                ...
                if !contacts.is_null() {
                    ...
                    ret = dc_mprintf(
                        b"%i contacts\nLocation streaming: %i\x00" as *const u8
                            as *const libc::c_char,
            ...
            while j < dc_array_get_cnt(loc) {
                ...
                dc_log_info(
                    context,
                    0i32,
                    b"Loc#%i: %s: lat=%f lng=%f acc=%f Chat#%i Contact#%i Msg#%i %s\x00"
                        as *const u8 as *const libc::c_char,
        ...
        ) == 0i32
        {
            if !sel_chat.is_null() {
                if !arg1.is_null() && 0 != *arg1.offset(0isize) as libc::c_int {
                    ...
                    ret = dc_strdup(
                        b"ERROR: No timeout given.\x00" as *const u8 as *const libc::c_char,
                    )
                }
            } else {
                ret = dc_strdup(b"No chat selected.\x00" as *const u8 as *const libc::c_char)
            ...
            if !arg1.is_null() && !arg2_1.is_null() {
                *arg2_1 = 0i32 as libc::c_char;
                ...
                ret = dc_strdup(if 0 != continue_streaming {
                    b"Success, streaming should be continued.\x00" as *const u8
                        as *const libc::c_char
                } else {
                ...
                ret = dc_strdup(
                    b"ERROR: Latitude or longitude not given.\x00" as *const u8
                        as *const libc::c_char,
                )
            }
            ...
            if !sel_chat.is_null() {
                if !arg1.is_null() && 0 != *arg1.offset(0isize) as libc::c_int {
                    if 0 != dc_send_text_msg(context, dc_chat_get_id(sel_chat), arg1) {
                        ...
                        ret = dc_strdup(
                            b"ERROR: Sending failed.\x00" as *const u8 as *const libc::c_char,
                        )
                    }
                } else {
                    ret = dc_strdup(
            ...
            || strcmp(cmd, b"sendfile\x00" as *const u8 as *const libc::c_char) == 0i32
        {
            if !sel_chat.is_null() {
                if !arg1.is_null() && 0 != *arg1.offset(0isize) as libc::c_int {
                    ...
                    ret =
                        dc_strdup(b"ERROR: No file given.\x00" as *const u8 as *const libc::c_char)
                }
            } else {
                ret = dc_strdup(b"No chat selected.\x00" as *const u8 as *const libc::c_char)
            }
            ...
            || strcmp(cmd, b"listverified\x00" as *const u8 as *const libc::c_char) == 0i32
        {
            ...
            if !arg1.is_null() {
                ...
                let mut contact: *mut dc_contact_t =
                    dc_get_contact(context, contact_id_3 as uint32_t);
                ...
                if chatlist_cnt > 0i32 {
                    ...
                    while i_1 < chatlist_cnt {
                        ...
                        let mut chat_1: *mut dc_chat_t = dc_get_chat(
                            context,
                            dc_chatlist_get_chat_id(chatlist_0, i_1 as size_t),
                        );
                        dc_strbuilder_catf(
                            &mut strbuilder as *mut dc_strbuilder_t,
        ...
        ) == 0i32
        {
            ...
            if !arg1.is_null() {
                ...
                ret = dc_strdup(
                    b"ERROR: Argument <qr-content> missing.\x00" as *const u8
                        as *const libc::c_char,
                )
            }
            ...
            if !arg1.is_null() {
                ...
                ret = dc_strdup(
                    b"ERROR: Argument <id> missing.\x00" as *const u8 as *const libc::c_char,
                )
            }
            ...
            if !arg1.is_null() {
                ...
                if 0 != dc_read_file(
                    ...
                    ret =
                        dc_strdup(b"ERROR: Command failed.\x00" as *const u8 as *const libc::c_char)
                }
